...
export class CoronavirusSelectComponent implements OnInit {
  ...
  @Output() readonly selectCountryEvent: EventEmitter<string> = new EventEmitter<string>(true);

  /* Region */
  @Input() selectedRegion;
  filteredRegions: any[] = FRANCE_REGIONS;
  @Output() readonly selectRegionEvent: EventEmitter<string> = new EventEmitter<string>(true);

  /* Region */
  @Input() selectedDepartment;
  filteredDepartments: any[] = FRANCE_DEPS;
  @Output() readonly selectDepartmentEvent: EventEmitter<string> = new EventEmitter<string>(true);
  ...
  filterCountries(value: string) {
    ...
    this.filteredCountries = COUNTRIES.filter(country => (country.translation.toLowerCase())
  ...
  filterDepartments(value: string) {
    ...
    this.filteredDepartments = FRANCE_DEPS.filter(department => (department.name.toLowerCase())
      .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
      .indexOf(search) > -1);
  }
